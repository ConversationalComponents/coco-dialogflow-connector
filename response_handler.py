...
def handle(dialogflow_response, response_time_seconds=0.0):
    """
    ...
    coco_standard_response["component_done"] = \
        (app.config.get("COMPLETE_ACTION") == action_name)

    coco_standard_response["component_failed"] = \
        (app.config.get("FAILED_ACTION") == action_name)

    coco_standard_response["out_of_context"] = \
        (app.config.get("OUT_OF_CONTEXT_ACTION") == action_name)
